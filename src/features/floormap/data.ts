...
export interface NDesk {
  ...
  y: number
  chair: 'top' | 'bottom' // which side the chair/person sits (inferred from bench pairing)
  personId?: string
  status: NStatus
  note?: string
}
...
// Infer chair facing: in a back-to-back bench the upper desk faces down (chair
// on top, away from its partner) and the lower desk faces up. Detect a vertical
// partner in the same column to set the side; the rest default to chair-on-top.
for (const d of BASE_DESKS) {
  if (d.seatType === 'cabin') continue
  const below = BASE_DESKS.some((e) => e !== d && Math.abs(e.x - d.x) < 26 && e.y - d.y > 14 && e.y - d.y < 78)
  const above = BASE_DESKS.some((e) => e !== d && Math.abs(e.x - d.x) < 26 && d.y - e.y > 14 && d.y - e.y < 78)
  d.chair = below && !above ? 'top' : above && !below ? 'bottom' : 'top'
}
