...
export function DeskDetail({
  ...
  const release = useFloorMap((s) => s.release)
  const setStatus = useFloorMap((s) => s.setStatus)
...
function RequestModal({
  ...
  const desks = useFloorMap((s) => s.desks)
  const personaId = useFloorMap((s) => s.personaId)
  const requestChange = useFloorMap((s) => s.requestChange)
  const requestSwap = useFloorMap((s) => s.requestSwap)
  const toast = useUI((s) => s.toast)
